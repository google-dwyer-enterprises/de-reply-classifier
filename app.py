...
def followups_playbook():
    # The data-grounded FU1–FU5 guide. Generated by the pipeline
    # (scripts/gen_followup_playbook.py, refreshed on the daily cron) and shipped
    # with the web image; served as a self-contained page (opens in a new tab
    # from the sidebar). It's a periodically-regenerated snapshot, not live.
    path = Path(__file__).resolve().parent / "docs" / "replies" / "FOLLOWUP_PLAYBOOK.html"
    if not path.exists():
        abort(404)
...
def category_booking():
    # "Which categories / titles book calls" — generated by
    # scripts/gen_category_booking_report.py (bundled into the daily report
    # regen) and shipped with the web image. A periodically-regenerated snapshot.
    path = Path(__file__).resolve().parent / "docs" / "replies" / "CATEGORY_BOOKING.html"
    if not path.exists():
        abort(404)
    return send_file(path)
