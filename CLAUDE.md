...
| `amazon_brand_match.py` | Strict SmartScout brand matcher (token_sort_ratio, NOT token_set_ratio). Kills containment false-positives: "Blue Apple Co." never matches brand "Apple". Three-tier: exact → fuzzy (FUZZY_HIGH=90) → LLM grey-zone (FUZZY_LOW=78, `use_llm=True` only). |
| `amazon_presence.py` | Rainforest API wrapper (1 credit/search). `rainforest_score()` → presence + revenue floor (sum of `bought in past month` × price × 12) + ratings + top_byline. `score_search_results()` is provider-agnostic. Playwright fallback is prototype-only (CAPTCHA risk at volume). |
| `amazon_revenue_qa.py` | **Amazon Revenue QA gate** — cascade: cache (90-day TTL) → SmartScout (free, ~20% hit) → Rainforest floor. Verdicts KEEP / DROP / REVIEW / PENDING_CREDITS. `floor_verdict()` + `rainforest_floor()` (with canonical-name requery). `qa_companies()` is the pipeline integration point (shadow mode; `AMAZON_QA_ENFORCE` in `bettercontact_sync.py` gates auto-drop). Never silently drops on API failure → REVIEW. |
| `amazon_revenue_qa_batch.py` | Standalone batch harness: `--dry-run` (zero credits, projects need), `--limit` + `--max-credits` live run (y/N gated), `--report-only`. Writes `debug/amazon_qa_batch_results.json` + `docs/scraping/AMAZON_QA_1K_REPORT.html`. |
| `helium10_revenue.py` | **Shelved** — Helium 10 UI automation evaluated and rejected (ToS risk; see `docs/scraping/HELIUM10_FEASIBILITY_RESEARCH.md`). File kept for reference; never called. |
