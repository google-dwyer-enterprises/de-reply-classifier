...
- **`sent_messages` table is currently empty in production** (verified 2026-05-19: 0 rows vs. 17,376 in `replies`). DDL exists in `migrations.sql:20` and `backfill_tags.py` references it for UPDATEs, but nothing inserts there yet. Plan to populate it lives in `FOLLOWUP_ANALYSIS_PLAN.md` — extends `instantly_sync.py` with an `email_type=sent` pass. Any query that reads from `sent_messages` without joining elsewhere returns nothing today.
...
- `FOLLOWUP_ANALYSIS_PLAN.md` — "which replies are working" analysis. Pulls outbound emails from Instantly into `sent_messages`, scores Unibox manual follow-ups via Haiku, exposes `followup_effectiveness_mv` to NocoDB. Phase 0 (API probe) done; Phase 1+ pending sign-off.
- `LEAD_CLEANING_PLAN.md` — Booked-count under-reporting fix (139→200+), name population, reply tracking view. Phase 1b resolved 2026-05-19 (sent_messages confirmed empty). Phase 4 now depends on FOLLOWUP_ANALYSIS_PLAN's sync work.
